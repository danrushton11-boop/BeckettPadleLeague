...
export default function Home() {
  ...
  const addMatch = () => {
    ...
    const updatedTeams = teams.map(t => {
      let tCopy = { ...t };
      if (t.name === winner) { tCopy.wins++; tCopy.points += 3; }
      else if (winner === 'Draw') { if (t.name === team1 || t.name === team2) { tCopy.draws++; tCopy.points += 1; } }
      else if (t.name === team1 || t.name === team2) { tCopy.losses++; }
      return tCopy;
    });
  ...
  const resetAll = () => { if (admin && confirm('Reset all data?')) { setMatches([]); setRatings({}); setTeams(initialTeams.map(t => ({ ...t, points: 0, wins:0, draws:0, losses:0 }))); setFixtures([]); } };
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        <h2>League Table</h2>
        <table>
          <thead><tr><th>Team</th><th>Points</th><th>Wins</th><th>Draws</th><th>Losses</th></tr></thead>
          <tbody>
            {teams.sort((a,b)=>b.points-a.points).map(t => <tr key={t.id}><td>{t.name}</td><td>{t.points}</td><td>{t.wins}</td><td>{t.draws}</td><td>{t.losses}</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      <div className="card">
        ...
        <table>
          ...
          <tbody>
            {fixtures.map((f,i) => <tr key={i}><td>{f.team1}</td><td>{f.team2}</td><td>{f.played?'Played':'Pending'}</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      <div className="card">
        ...
        <table>
          <thead><tr><th>Player</th><th>Match</th><th>Rating</th><th>Average</th></tr></thead>
          <tbody>
